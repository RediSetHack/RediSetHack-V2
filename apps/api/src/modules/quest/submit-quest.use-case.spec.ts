...
const quest = new Quest(1, 10, "Loops 101", "Test your loop knowledge", 60, 70, 50);
...
function makeQuestRepo(overrides: Partial<QuestRepository> = {}): QuestRepository {
  return {
    findAll: vi.fn(),
    findById: vi.fn().mockResolvedValue(quest),
    findQuestions: vi.fn().mockResolvedValue(questions),
    findPassedQuestIds: vi.fn(),
    createResult: vi.fn().mockImplementation(
      async (input) => new QuestResult(1, input.userId, input.questId, input.score, input.passed, input.responses, new Date()),
    ),
...
function makeUserRepo(overrides: Partial<UserRepository> = {}): UserRepository {
  return {
    ...
    awardXp: vi.fn().mockImplementation(
      async (userId, amount) => new User(userId, "learner@example.com", "Learner", null, amount),
    ),
    ...overrides,
  };
}
...
function makeGetTodayEvent(multiplier = 1): GetTodayEventUseCase {
  const repo: DailyEventRepository = {
    findByDate: vi.fn().mockResolvedValue(
      new DailyEvent(1, "2026-09-12", multiplier > 1 ? "bonus" : "normal", multiplier),
    ),
    create: vi.fn(),
  };
...
describe("SubmitQuestUseCase", () => {
  it("scores correct responses and marks the attempt as passed", async () => {
    ...
    const useCase = new SubmitQuestUseCase(quests, users, makeGetTodayEvent(), makeEvaluateBadges());
  ...
  it("doubles the XP award on a Bonus event day", async () => {
    const quests = makeQuestRepo();
    const users = makeUserRepo();
    const useCase = new SubmitQuestUseCase(quests, users, makeGetTodayEvent(2), makeEvaluateBadges());
  ...
  it("awards zero XP on a passing retake", async () => {
    const quests = makeQuestRepo({ claimXpAward: vi.fn().mockResolvedValue(false) });
    const users = makeUserRepo();
    const useCase = new SubmitQuestUseCase(quests, users, makeGetTodayEvent(), makeEvaluateBadges());
  ...
  it("awards no XP when the quest carries no xpReward", async () => {
    ...
    const quests = makeQuestRepo({ findById: vi.fn().mockResolvedValue(zeroRewardQuest) });
    const users = makeUserRepo();
    const useCase = new SubmitQuestUseCase(quests, users, makeGetTodayEvent(), makeEvaluateBadges());
