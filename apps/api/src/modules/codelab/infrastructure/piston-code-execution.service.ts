...
export class PistonCodeExecutionService implements CodeExecutionPort {
  ...
  constructor(config: ConfigService) {
    this.baseUrl =
      config.get<string>('CODE_EXECUTION_API_URL') ?? DEFAULT_PISTON_URL;
  }
  ...
  async execute(request: ExecutionRequest): Promise<ExecutionResult> {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Code execution provider request failed (${response.status}): ${body}`,
      );
    }
    ...
    if (!result.run) {
      throw new Error(
        result.message ?? 'Code execution provider returned no run result',
      );
    }
