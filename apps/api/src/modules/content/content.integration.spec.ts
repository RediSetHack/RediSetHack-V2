...
import { ClerkAuthPort, type ClerkAuthenticatedUser } from "../auth/domain/ports/clerk-auth.port.js";
...
function makeRegionRepository(): RegionRepository {
  ...
  return {
    ...
    async create(input) {
      const region = new Region(nextId++, input.name, input.slug, input.description, input.sortOrder);
...
function makeZoneRepository(): ZoneRepository {
  ...
  return {
    ...
    async create(input) {
      const zone = new Zone(nextId++, input.regionId, input.name, input.slug, input.description, input.sortOrder);
  ...
  return {
    ...
    async update(id, input) {
      ...
      const updated = new Stage(
        ...
        input.slug ?? existing.slug,
        input.lessonContent !== undefined ? input.lessonContent : existing.lessonContent,
...
function makeBadgeDefinitionRepository(): BadgeDefinitionRepository {
  ...
  return {
    ...
    async create(input) {
      const badge = new BadgeDefinition(nextId++, input.name, input.slug, input.description, input.criteria, input.imageUrl);
...
function makeCharacterRepository(): CharacterRepository {
  ...
  return {
    ...
    async create(input) {
      const character = new Character(nextId++, input.name, input.slug, input.description, input.imageUrl);
...
describe("content admin CRUD", () => {
  ...
  const fakeClerk: ClerkAuthPort = {
    async authenticate(request: IncomingMessage) {
      if (request.headers["authorization"] !== "Bearer valid-token") return null;
  ...
  beforeEach(async () => {
    ...
    const moduleRef = await Test.createTestingModule({
      ...
      providers: [
        ...
        AdminGuard,
        EnsureUserUseCase,
        { provide: CreateRegionUseCase, useFactory: () => new CreateRegionUseCase(regions) },
        { provide: GetRegionUseCase, useFactory: () => new GetRegionUseCase(regions) },
        { provide: UpdateRegionUseCase, useFactory: () => new UpdateRegionUseCase(regions) },
        { provide: DeleteRegionUseCase, useFactory: () => new DeleteRegionUseCase(regions) },
        { provide: CreateZoneUseCase, useFactory: () => new CreateZoneUseCase(zones, regions) },
        { provide: GetZoneUseCase, useFactory: () => new GetZoneUseCase(zones) },
        { provide: UpdateZoneUseCase, useFactory: () => new UpdateZoneUseCase(zones) },
        { provide: DeleteZoneUseCase, useFactory: () => new DeleteZoneUseCase(zones) },
        { provide: CreateStageUseCase, useFactory: () => new CreateStageUseCase(stages, zones) },
        { provide: GetStageUseCase, useFactory: () => new GetStageUseCase(stages) },
        { provide: UpdateStageUseCase, useFactory: () => new UpdateStageUseCase(stages) },
        { provide: DeleteStageUseCase, useFactory: () => new DeleteStageUseCase(stages) },
        { provide: CreateQuestUseCase, useFactory: () => new CreateQuestUseCase(quests, stages) },
        { provide: GetQuestUseCase, useFactory: () => new GetQuestUseCase(quests) },
        { provide: UpdateQuestUseCase, useFactory: () => new UpdateQuestUseCase(quests) },
        { provide: DeleteQuestUseCase, useFactory: () => new DeleteQuestUseCase(quests) },
        { provide: CreateBadgeDefinitionUseCase, useFactory: () => new CreateBadgeDefinitionUseCase(badges) },
        { provide: GetBadgeDefinitionUseCase, useFactory: () => new GetBadgeDefinitionUseCase(badges) },
        { provide: UpdateBadgeDefinitionUseCase, useFactory: () => new UpdateBadgeDefinitionUseCase(badges) },
        { provide: DeleteBadgeDefinitionUseCase, useFactory: () => new DeleteBadgeDefinitionUseCase(badges) },
        { provide: CreateCharacterUseCase, useFactory: () => new CreateCharacterUseCase(characters) },
        { provide: GetCharacterUseCase, useFactory: () => new GetCharacterUseCase(characters) },
        { provide: UpdateCharacterUseCase, useFactory: () => new UpdateCharacterUseCase(characters) },
        { provide: DeleteCharacterUseCase, useFactory: () => new DeleteCharacterUseCase(characters) },
      ],
    }).compile();
  ...
  const admin = () => ({ authorization: "Bearer valid-token", "x-probe-role": "admin" });
  const learner = () => ({ authorization: "Bearer valid-token", "x-probe-role": "user" });

  it("rejects unauthenticated requests with 401", async () => {
    const res = await request(app.getHttpServer()).post("/v1/api/admin/regions").send({ name: "Basics", slug: "basics" });
  ...
  it("runs the full CRUD lifecycle for regions", async () => {
    ...
    const get = await request(app.getHttpServer()).get(`/v1/api/admin/regions/${id}`).set(admin());
    ...
    const del = await request(app.getHttpServer()).delete(`/v1/api/admin/regions/${id}`).set(admin());
    expect(del.status).toBe(204);

    const getAfterDelete = await request(app.getHttpServer()).get(`/v1/api/admin/regions/${id}`).set(admin());
  ...
  it("runs the full CRUD lifecycle for zones nested under a region", async () => {
    const region = await request(app.getHttpServer()).post("/v1/api/admin/regions").set(admin()).send({ name: "Basics", slug: "basics" });
    ...
    const update = await request(app.getHttpServer()).patch(`/v1/api/admin/zones/${id}`).set(admin()).send({ sortOrder: 5 });
    expect(update.status).toBe(200);
    expect(update.body.sortOrder).toBe(5);

    const del = await request(app.getHttpServer()).delete(`/v1/api/admin/zones/${id}`).set(admin());
    expect(del.status).toBe(204);
  });
  ...
  it("validates structured lesson content when creating a stage", async () => {
    ...
    const invalid = await request(app.getHttpServer())
      ...
      .set(admin())
      .send({ zoneId: zone.body.id, title: "For loops", slug: "for-loops", lessonContent: [{ type: "bogus" }] });
    ...
    expect(valid.body.lessonContent).toEqual([{ type: "text", content: "Loops repeat code." }]);
  });
  ...
  it("validates question schemas and answer definitions when creating a quest", async () => {
    ...
    const invalid = await request(app.getHttpServer())
      ...
      .send({
        ...
        title: "Loop quiz",
        questions: [{ id: "q1", prompt: "?", options: [{ id: "a", text: "A" }], correctOptionId: "z" }],
      });
    expect(invalid.status).toBe(400);
    ...
    const get = await request(app.getHttpServer()).get(`/v1/api/admin/quests/${valid.body.id}`).set(admin());
    expect(get.status).toBe(200);

    const del = await request(app.getHttpServer()).delete(`/v1/api/admin/quests/${valid.body.id}`).set(admin());
    expect(del.status).toBe(204);
  });
  ...
  it("validates badge trigger criteria and runs the badge CRUD lifecycle", async () => {
    const invalid = await request(app.getHttpServer())
      ...
      .set(admin())
      .send({ name: "Streaker", slug: "streaker", criteria: { trigger: "bogus", target: "x", threshold: 1 } });
    expect(invalid.status).toBe(400);
    ...
    const create = await request(app.getHttpServer())
      ...
      .send({
        name: "Streaker",
        slug: "streaker",
        criteria: { trigger: "cumulative", target: "stage_completions", threshold: 5 },
      });
    expect(create.status).toBe(201);
    const id = create.body.id;
    ...
    const update = await request(app.getHttpServer())
      ...
      .set(admin())
      .send({ criteria: { trigger: "cumulative", target: "stage_completions", threshold: 10 } });
    expect(update.status).toBe(200);
    expect(update.body.criteria.threshold).toBe(10);

    const del = await request(app.getHttpServer()).delete(`/v1/api/admin/badges/${id}`).set(admin());
    expect(del.status).toBe(204);
  });
  ...
  it("runs the full CRUD lifecycle for characters", async () => {
    ...
    const del = await request(app.getHttpServer()).delete(`/v1/api/admin/characters/${id}`).set(admin());
    expect(del.status).toBe(204);

    const getAfterDelete = await request(app.getHttpServer()).get(`/v1/api/admin/characters/${id}`).set(admin());
    expect(getAfterDelete.status).toBe(404);
  });
});
