...
describe('EvaluateBadgesUseCase', () => {
  it('awards a cumulative badge once its stage-completion threshold is crossed', async () => {
    ...
    expect(awarded).toEqual([
      { badge: cumulativeBadge, awardCount: 1, newAwards: 1 },
    ]);
  ...
  it('increments the award count multiple times once several thresholds are crossed at once', async () => {
    ...
    expect(awarded).toEqual([
      { badge: cumulativeBadge, awardCount: 3, newAwards: 2 },
    ]);
  ...
  it('awards an activity badge based on total passed-quest count', async () => {
    const repo = makeRepo({
    ...
    expect(awarded).toEqual([
      { badge: activityBadge, awardCount: 1, newAwards: 1 },
    ]);
  });
  ...
  it('awards a category badge once when its target zone becomes fully completed', async () => {
    ...
    expect(awarded).toEqual([
      { badge: categoryBadge, awardCount: 1, newAwards: 1 },
    ]);
  ...
  it('evaluates every active badge definition independently in one pass', async () => {
    const repo = makeRepo({
      findAll: vi
        .fn()
        .mockResolvedValue([cumulativeBadge, activityBadge, categoryBadge]),
    ...
    expect(awarded).toEqual([
      { badge: cumulativeBadge, awardCount: 2, newAwards: 2 },
    ]);
  });
});
