...
export abstract class BadgeRepository {
  ...
  abstract countAwards(userId: string, badgeDefinitionId: number): Promise<number>;
  // Inserts `count` new award records, each a distinct timestamped occurrence.
  abstract awardMany(userId: string, badgeDefinitionId: number, count: number): Promise<void>;
